...
def generate_pdf(employee: Employee, review_content: str, quarter: str, year: int, line_graph=None) -> bytes:
    ...
    # Add line graph as second page if available
    if line_graph and line_graph.get('file_data'):
        ...
        try:
            ...
            # Add header for line graph page
            story.append(Spacer(1, 20))
            story.append(Paragraph("🦐 BUBBA GUMP SHRIMP CO. 🦐", title_style))
            story.append(Paragraph("Restaurant & Market • Las Vegas", subtitle_style))
            story.append(Spacer(1, 10))
            
            story.append(Paragraph(f"QUARTERLY PERFORMANCE TRENDS - {quarter} {year}", header_style))
            story.append(Paragraph(f"Employee: {employee.name} • Position: {employee.position}", 
            ...
            logging.error(f"Error adding line graph to PDF: {str(e)}")
